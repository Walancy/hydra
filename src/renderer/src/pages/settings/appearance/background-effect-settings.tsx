...
import {
  IconNone, IconDarkVeil, IconLightPillar, IconFloatingLines,
  IconLightRays, IconColorBends, IconParticles, IconBeams, IconPixelBlast,
} from "./effect-icons";
...
> = {
  ...
  floatinglines: {
    ...
    defaults: { linesGradient: ["#a5cfaa", "#273f3b"], lineCount: 5, lineDistance: 5, bendRadius: 5, bendStrength: -0.5, interactive: true, parallax: true },
    info: "Linhas flutuantes responsivas ao mouse",
  },
  lightrays: {
    label: "Light Rays",
    defaults: { raysOrigin: "top-center", raysColor: "#ffffff", raysSpeed: 1, lightSpread: 0.5, rayLength: 3, followMouse: true, mouseInfluence: 0.1, noiseAmount: 0, pulsating: false, fadeDistance: 1, saturation: 1 },
  ...
  particles: {
    label: "Particles",
    defaults: { particleColors: ["#ffffff"], particleCount: 200, particleSpread: 10, speed: 0.1, particleBaseSize: 1, moveParticlesOnHover: true, alphaParticles: false, disableRotation: false },
  ...
  pixelblast: {
    label: "Pixel Blast",
    defaults: { color: "#07e874", pixelSize: 3, patternScale: 3.5, patternDensity: 1.6, rippleSpeed: 0.3, rippleThickness: 0.07, rippleIntensityScale: 1.2, speed: 0.4, transparent: true, edgeFade: 0.4, enableRipples: true, variant: "square" },
    info: "Partículas pixeladas interativas (estilo Matrix)",
  },
};

// ── sub-components ─────────────────────────────────────────────────────────────
interface ToggleProps { id: string; checked: boolean; onChange: (v: boolean) => void }
...
interface SliderProps { label: string; value: number; range: PropRange; onChange: (v: number) => void }
...
export function BackgroundEffectSettings() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setConfig(confStr ? JSON.parse(confStr) : effectsInfo["floatinglines"].defaults);
    } catch { setConfig({}); }
  }, []);
  ...
  const entries = Object.entries(currentInfo.defaults).filter(([k]) => !skipped.includes(k));

  const colorProps  = entries.filter(([,dv]) => (typeof dv === "string" && (dv as string).startsWith("#")) || (Array.isArray(dv) && (dv as string[])[0]?.startsWith("#")));
  const numberProps = entries.filter(([k, dv]) => typeof dv === "number" && !radios[k]);
  const boolProps   = entries.filter(([,dv]) => typeof dv === "boolean");
  const radioProps  = Object.entries(radios);
  const stringProps = entries.filter(([k,dv]) => typeof dv === "string" && !(dv as string).startsWith("#") && !radios[k]);
    ...
    <div className="bg-effect">
      ...
      <div className="bg-effect__selectors">
        {Object.entries(effectsInfo).map(([key, info]) => (
          <Button key={key} theme={effect === key ? "primary" : "outline"}
            className="bg-effect__btn" onClick={() => handleEffectChange(key)}>
            ...
            <Section title="Cores">
              <div className="bg-effect__section-body">
                {colorProps.map(([key, defaultValue]) => {
                  const val = (config[key] ?? defaultValue) as string | string[];
                          ...
                          {val.map((color, i) => (
                            <div key={i} className="bg-effect__color-row">
                              <div className="bg-effect__color-input">
                                <input type="color" value={color.substring(0,7)} onChange={(e) => { const n=[...val]; n[i]=e.target.value; handleConfigChange(key,n); }} />
                                <TextField value={color} onChange={(e) => { const n=[...val]; n[i]=e.target.value; handleConfigChange(key,n); }} />
                              </div>
                              <button type="button" className="bg-effect__remove-color" onClick={() => { const n=val.filter((_,j)=>j!==i); if(n.length>0) handleConfigChange(key,n); }}>×</button>
                            </div>
                          ))}
                          <Button theme="outline" className="bg-effect__add-color" onClick={() => handleConfigChange(key,[...val,"#ffffff"])}>+ Adicionar Cor</Button>
                        </div>
                      </div>
                    );
                  }
                  return (
                    <div key={key} className="bg-effect__prop">
                      <label>{humanize(key)}</label>
                      <div className="bg-effect__color-input">
                        <input type="color" value={(val as string).substring(0,7)} onChange={(e) => handleConfigChange(key, e.target.value)} />
                        <TextField value={val as string} onChange={(e) => handleConfigChange(key, e.target.value)} />
                      </div>
                    </div>
                  );
                })}
            ...
            <Section title="Ajustes">
              <div className="bg-effect__section-grid">
                {numberProps.map(([key, defaultValue]) => {
                  ...
                  return <SliderProp key={key} label={humanize(key)} value={val} range={range} onChange={(v) => handleConfigChange(key,v)} />;
                })}
              </div>
            </Section>
          )}
            ...
            <Section title="Posição / Tipo">
              <div className="bg-effect__section-body">
                {radioProps.map(([key, options]) => {
                  const val = (config[key] ?? currentInfo.defaults[key]) as string;
                  return (
                    <div key={key} className="bg-effect__prop">
                      <label>{humanize(key)}</label>
                      <div className="bg-effect__radio-group">
                        {options.map((opt) => (
                          <label key={opt} className={`bg-effect__radio ${val===opt ? "bg-effect__radio--active" : ""}`}>
                            <input type="radio" name={`${uid}-${key}`} value={opt} checked={val===opt} onChange={() => handleConfigChange(key,opt)} />
                            {opt}
                          </label>
                        ))}
                      </div>
              ...
              <div className="bg-effect__section-body">
                {stringProps.map(([key, defaultValue]) => {
                  ...
                  return (
                    <div key={key} className="bg-effect__prop">
                      <label>{humanize(key)}</label>
                      <TextField value={val} onChange={(e) => handleConfigChange(key, e.target.value)} />
                    </div>
                  );
                })}
              </div>
            ...
            <Section title="Comportamento">
              <div className="bg-effect__section-body">
                {boolProps.map(([key, defaultValue]) => {
                  ...
                  return (
                    <div key={key} className="bg-effect__prop bg-effect__prop--toggle">
                      <label htmlFor={inputId}>{humanize(key)}</label>
                      <Toggle id={inputId} checked={val} onChange={(v) => handleConfigChange(key,v)} />
                    </div>
                  );
                })}
              </div>
