...
export function DecorationPickerModal({
  ...
  const [activeCategory, setActiveCategory] = useState<DecorationCategory>(DECORATION_CATEGORIES[0]);
  ...
  useEffect(() => {
    if (validCategories.length > 0 && !validCategories.includes(activeCategory)) {
      setActiveCategory(validCategories[0]);
    }
  }, [validCategories, activeCategory]);

  const filtered = useMemo(() => {
    const byCategory = AVATAR_DECORATIONS.filter((d) => d.category === activeCategory && !brokenIds.has(d.id));
    ...
    >
      <div className="decoration-picker">
        <aside className="decoration-picker__sidebar">
          ...
          <nav className="decoration-picker__categories" aria-label="Categorias de decoração">
        ...
        <main className="decoration-picker__main">
          <header className="decoration-picker__main-header">
            <h3 className="decoration-picker__main-title">{CATEGORY_LABELS[activeCategory]}</h3>
            <span className="decoration-picker__count">
              {filtered.length} decoraç{filtered.length === 1 ? "ão" : "ões"}
            </span>
          </header>

          <div className="decoration-picker__grid" role="listbox" aria-label="Decorações disponíveis">
            {filtered.length === 0 && (
              <p className="decoration-picker__empty">Nenhuma decoração encontrada.</p>
            )}
            {filtered.map((decor) => (
              <button
                key={decor.id}
              ...
              >
                <div className="decoration-picker__item-preview">
                  ...
                  <img
                    ...
                    src={hoveredId === decor.id ? getDecorationUrl(decor.id) : getDecorationPreviewUrl(decor.id)}
                    alt={decor.label}
                    loading="lazy"
                    onError={() => markBroken(decor.id)}
                  />
                </div>
                <span className="decoration-picker__item-label">{decor.label}</span>
              </button>
            ))}
          </div>
        </main>
